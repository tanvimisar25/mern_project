...
const initialFlashcardQuestions = [
  { id: "ss_1", deckId: "ssa", front: "What is the single most important prerequisite for performing a binary search on an array?", back: "The array must be sorted. Binary search works by repeatedly dividing the search interval in half, which is only possible if the elements are in a known order." },
{ id: "ss_2", deckId: "ssa", front: "Explain the 'Divide and Conquer' strategy and name two sorting algorithms that use it.", back: "It's a strategy that breaks a problem into smaller, more manageable subproblems, solves them recursively, and then combines their solutions. Merge Sort and Quick Sort are classic examples." },
{ id: "ss_3", deckId: "ssa", front: "What is the worst-case time complexity of Quick Sort and what causes it?", back: "The worst-case complexity is O(n^2). This occurs when the chosen pivot is consistently the smallest or largest element, leading to an unbalanced partition where one subproblem has n-1 elements and the other has 0." },
{ id: "ss_4", deckId: "ssa", front: "What does it mean for a sorting algorithm to be 'stable'?", back: "A sorting algorithm is stable if it preserves the original relative order of equal elements. For example, if two items have the same key, their order in the input will be the same as their order in the output." },
{ id: "ss_5", deckId: "ssa", front: "How does Bubble Sort work?", back: "It repeatedly steps through the list, compares adjacent elements, and swaps them if they are in the wrong order. This process is repeated until the list is sorted, with larger elements 'bubbling' to the end with each pass." },
{ id: "ss_6", deckId: "ssa", front: "Which sorting algorithm is most efficient for an array that is already 'mostly sorted'?", back: "Insertion Sort is highly efficient for mostly sorted data, with a time complexity that approaches O(n) because it requires very few swaps or shifts." },
{ id: "ss_17", deckId: "ssa", front: "What is the primary difference between Merge Sort and Quick Sort's space complexity?", back: "Merge Sort requires O(n) auxiliary space to merge sorted subarrays, making it out-of-place. Quick Sort typically has O(log n) space for recursion and sorts in-place." },
{ id: "ss_8", deckId: "ssa", front: "How does Selection Sort work?", back: "It divides the list into a sorted sublist and an unsorted sublist. Repeatedly finds the minimum element from the unsorted sublist and moves it to the end of the sorted sublist." },
{ id: "ss_9", deckId: "ssa", front: "What data structure is used by Heap Sort?", back: "Heap Sort uses a binary heap. It builds a max-heap, then repeatedly extracts the maximum element from the root and rebuilds the heap until the array is sorted." },
{ id: "ss_10", deckId: "ssa", front: "Why is Binary Search generally faster than Linear Search?", back: "Binary Search has O(log n) time complexity because it eliminates half the remaining data each step. Linear Search is O(n), checking each element, making it slower for large datasets." }
...
function SearchSort() {
    ...
    const personalizedQuestions = initialFlashcardQuestions.map(q => {
        const deckEdits = userProfile.editedDecks[q.deckId];  // e.g., "ssa"
